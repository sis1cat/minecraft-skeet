...
public class Selectable extends Widget{
    ...
    public Selectable(FunctionSetting functionSetting, String text, Object link, int tab){
    ...
    public Selectable(FunctionSetting functionSetting, String text, int tab){
        this.functionSetting = functionSetting;
        this.text = text;
        this.link = null;
        this.tab = tab;
    ...
    public void draw(int x, int y, int groupWidth) {
        ...
        ArrayList<String> selectedOptionsList = functionSetting.getSelectedOptionsList();
        ...
        for(int i = 0; i < selectedOptionsList.size(); i++){
            ...
            selectedOptions +=
                    selectedOptionsList.get(i) +
                    (selectedOptionsList.size() - 1 != i ? ", " : "");

        }

        while((int) Math.round(6 * Window.windowScale) + Text.getMenuFont().getStringWidth(selectedOptions) > width - (int) Math.round(15 * Window.windowScale))
            selectedOptions = selectedOptions.substring(0, selectedOptions.toCharArray().length - 2);
        ...
        Render.writeTriangle2(
                ...
                this.y + (int) Math.round((float) widgetHeight / 2) - 3,
                5, 3,
                new float[]{0, 0, 0},
                alpha
        );
    ...
    public void drawOptions(){
        ...
        Render.writeRectangleBorders(this.x, this.y + widgetHeight + 1, width, height, 1, Color.c12, Widget.menuAlpha);
        Render.writeRectangle(this.x + 1, this.y + widgetHeight + 2, width - 2, height - 2, new float[]{35, 35, 35, 255}, Widget.menuAlpha);
        ...
        if(hoveredOption != -1) {
            ...
            Render.writeRectangle(
                    this.x + 1,
                    buttonY,
                    width - 2, buttonHeight, new float[]{25, 25, 25}, Widget.menuAlpha);
        ...
        for(int i = 0; i < functionSetting.getOptionsList().size(); i++) {
            ...
            Text.getMenuFont(hoveredOption == i || functionSetting.getSelectedOptionsList().contains(functionSetting.getOptionsList().get(i))).renderVCenteredText(
                    options.get(i),
                    this.x + Math.round(6 * Window.windowScale),
                    textY,
                    functionSetting.getSelectedOptionsList().contains(options.get(i)) ? Widget.themeColor : Color.c205
            );
    ...
    void _event(MouseEvent mouseEvent){
        ...
        if(hoveredOption != -1 && mouseEvent.getButton() == 0 && mouseEvent.getAction() == 0) {
            ...
            ArrayList<String>
                    optionsList = functionSetting.getOptionsList(),
                    selectedOptionsList = functionSetting.getSelectedOptionsList();

            String hoveredOptionName = optionsList.get(hoveredOption);

            if(selectedOptionsList.contains(hoveredOptionName))
                selectedOptionsList.remove(hoveredOptionName);
            else
                selectedOptionsList.add(hoveredOptionName);
