    ...
    public static ModelPart parseModelRenderer(JsonObject elem, Model modelBase, int[] parentTextureSize, String basePath) {
        ...
        if (jsonarray != null) {
            for (int j = 0; j < jsonarray.size(); j++) {
                ...
                float[] afloat5 = Json.parseFloatArray(jsonobject.get("sizesAdd"), 3);
                if (afloat5 == null) {
                    float f1 = Json.getFloat(jsonobject, "sizeAdd", 0.0F);
                    afloat5 = new float[]{f1, f1, f1};
                }

                if (afloat3 != null) {
                    modelpart.addBox(afloat3, afloat4[0], afloat4[1], afloat4[2], afloat4[3], afloat4[4], afloat4[5], afloat5[0], afloat5[1], afloat5[2]);
                } else {
        ...
        if (jsonarray1 != null) {
            for (int k = 0; k < jsonarray1.size(); k++) {
                ...
                float[] afloat6 = Json.parseFloatArray(jsonobject2.get("coordinates"), 6);
                if (afloat6 == null) {
                    throw new JsonParseException("Coordinates not specified");
                }

                if (flag) {
                    afloat6[0] = -afloat6[0] - afloat6[3];
                }

                if (flag1) {
                    afloat6[1] = -afloat6[1] - afloat6[4];
                }

                if (flag2) {
                    afloat6[2] = -afloat6[2] - afloat6[5];
                }

                float f2 = Json.getFloat(jsonobject2, "sizeAdd", 0.0F);
                modelpart.setTextureOffset((float)aint1[0], (float)aint1[1]);
                modelpart.addSprite(afloat6[0], afloat6[1], afloat6[2], (int)afloat6[3], (int)afloat6[4], (int)afloat6[5], f2);
            }
        }

        JsonObject jsonobject1 = (JsonObject)elem.get("submodel");
        if (jsonobject1 != null) {
            ModelPart modelpart1 = parseModelRenderer(jsonobject1, modelBase, aint, basePath);
            modelpart.addChildModel(getNextModelId(), modelpart1);
        }
        ...
        if (jsonarray2 != null) {
            for (int l = 0; l < jsonarray2.size(); l++) {
                ...
                ModelPart modelpart2 = parseModelRenderer(jsonobject3, modelBase, aint, basePath);
                if (modelpart2.getId() != null) {
                    ModelPart modelpart3 = modelpart.getChildById(modelpart2.getId());
                    if (modelpart3 != null) {
                        Config.warn("Duplicate model ID: " + modelpart2.getId());
                    }
                }
