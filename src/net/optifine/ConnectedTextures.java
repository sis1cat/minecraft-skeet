...
public class ConnectedTextures {
    ...
    private static BakedQuad makeSpriteQuad(BakedQuad quad, TextureAtlasSprite sprite) {
        ...
        return new BakedQuad(aint, quad.getTintIndex(), quad.getDirection(), sprite, quad.isShade(), quad.getLightEmission());
    }
    ...
    private static void fixVertex(int[] data, int vertex, TextureAtlasSprite spriteFrom, TextureAtlasSprite spriteTo) {
        ...
        data[j + 4] = Float.floatToRawIntBits(spriteTo.getInterpolatedU16(d0));
        data[j + 4 + 1] = Float.floatToRawIntBits(spriteTo.getInterpolatedV16(d1));
    }
        ...
        if (!cp.matchesBlock(blockState.getBlockId(), j)) {
            ...
            if (cp.heights != null && !cp.heights.isInRange(l)) {
                ...
                switch (cp.method) {
                    case 1:
                        return getQuads(
                            getConnectedTextureCtm(cp, blockAccess, blockState, blockPos, i, side, textureatlassprite, j, renderEnv), quad, renderEnv
                        );
                    case 2:
    ...
    private static TextureAtlasSprite getConnectedTextureRandom(
        ...
        if (cp.tileIcons.length == 1) {
            ...
            if (cp.linked) {
                ...
                for (BlockState blockstate = blockAccess.getBlockState(blockpos);
        ...
        if (!isFullCubeModel(blockstate, worldReader, blockPos)) {
            ...
            if (blockstate1.isSolidRender()) {
                ...
                return side == 1 && blockstate1.getBlock() == Blocks.SNOW
                    ? false
                    : !isNeighbour(cp, worldReader, blockState, blockPos, blockstate, side, icon, metadata);
            }
        }
    }
    ...
    private static boolean isFullCubeModel(BlockState state, BlockGetter blockReader, BlockPos pos) {
        if (BlockUtils.isFullCube(state, blockReader, pos)) {
            ...
            return block == Blocks.GLASS ? true : block instanceof StainedGlassBlock;
        }
    }
    ...
    private static boolean isNeighbourMatching(
        ...
        if (blockstate == AIR_DEFAULT_STATE) {
            ...
            BlockPos blockpos = blockPos.relative(getFacing(side));
            BlockState blockstate1 = worldReader.getBlockState(blockpos);
            return blockstate1.isSolidRender() ? false : side != 1 || blockstate1.getBlock() != Blocks.SNOW;
        }
    }
        ...
        if (bakedmodel == null) {
            ...
            if (list == null) {
                ...
                if (list.size() > 0) {
                    ...
                    List list1 = bakedmodel.getQuads(neighbourState, null, RANDOM);
                    if (list1 == null) {
                        return null;
                    } else {
    ...
    private static ConnectedProperties[][] propertyListToArray(List list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (listx != null) {
                ConnectedProperties[] aconnectedproperties1 = (ConnectedProperties[]) listx.toArray(new ConnectedProperties[listx.size()]);
