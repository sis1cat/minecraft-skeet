...
public class LightMapPack {
    ...
    public boolean updateLightmap(ClientLevel world, float torchFlickerX, NativeImage lmColorsImage, boolean nightvision, float darkLight, float partialTicks) {
        ...
        boolean flag = this.updateLightmap(world, torchFlickerX, this.lmColorsBuffer, nightvision, darkLight, partialTicks);
        if (flag) {
    ...
    public boolean updateLightmap(ClientLevel world, float torchFlickerX, int[] lmColors, boolean nightvision, float darkLight, float partialTicks) {
        if (this.lightMapRain == null && this.lightMapThunder == null) {
            ...
            if (!flag && !flag1) {
                ...
                if (f3 > f2 && this.lightMap.updateLightmap(world, torchFlickerX, aint[i], nightvision, darkLight)) {
                    afloat[i] = f3;
                    i++;
                }

                if (f4 > f2 && this.lightMapRain != null && this.lightMapRain.updateLightmap(world, torchFlickerX, aint[i], nightvision, darkLight)) {
                    afloat[i] = f4;
                    i++;
                }

                if (f1 > f2 && this.lightMapThunder != null && this.lightMapThunder.updateLightmap(world, torchFlickerX, aint[i], nightvision, darkLight)) {
                    afloat[i] = f1;
                    i++;
                }
