...
public class Vec2Argument implements ArgumentType<Coordinates> {
    ...
    public Coordinates parse(StringReader pReader) throws CommandSyntaxException {
        ...
        if (!pReader.canRead()) {
            ...
            WorldCoordinate worldcoordinate = WorldCoordinate.parseDouble(pReader, this.centerCorrect);
            if (pReader.canRead() && pReader.peek() == ' ') {
                pReader.skip();
                WorldCoordinate worldcoordinate1 = WorldCoordinate.parseDouble(pReader, this.centerCorrect);
