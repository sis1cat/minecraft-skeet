...
public class WorldUnloadEvent {
    ...
    public void setTime(long pTime) {
        if (this.lastGameTime != -1L) {
            this.totalTicks = this.totalTicks + Math.max(0L, pTime - this.lastGameTime);
        }
    ...
    public void send(TelemetryEventSender pSender) {
        this.worldLoadedTime.ifPresent(p_261953_ -> pSender.send(TelemetryEventType.WORLD_UNLOADED, p_261597_ -> {
