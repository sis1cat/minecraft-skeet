...
import sisicat.main.functions.visual.CustomItemRenderer;
...
public class ItemEntityRenderer extends EntityRenderer<ItemEntity, ItemEntityRenderState> {
    ...
    public void render(ItemEntityRenderState p_365095_, PoseStack p_115030_, MultiBufferSource p_115031_, int p_115032_) {
        ...
        if (!p_365095_.item.isEmpty()) {
            ...
            CustomItemRenderer.IER_render IER_render = new CustomItemRenderer.IER_render(this, p_365095_, p_115030_, p_115031_, p_115032_);
            EventManager.call(IER_render);

            if(!IER_render.isCancelled()) {
    ...
    public static void renderMultipleFromCount(PoseStack pPoseStack, MultiBufferSource pBufferSource, int pPackedLight, ItemClusterRenderState pRenderState, RandomSource pRandom) {
        ...
        CustomItemRenderer.IER_renderMultipleFromCount IER_renderMultipleFromCount = new CustomItemRenderer.IER_renderMultipleFromCount(pPoseStack, pBufferSource, pPackedLight, pRenderState, pRandom);
        EventManager.call(IER_renderMultipleFromCount);

        if(IER_renderMultipleFromCount.isCancelled())
            return;
