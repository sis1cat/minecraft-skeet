...
public class ChatLog {
        ...
        return Codec.list(LoggedChatEvent.CODEC)
            .comapFlatMap(
                p_274704_ -> {
                    ...
                    return i > pSize
                        ? DataResult.error(
                            () -> "Expected: a buffer of size less than or equal to " + pSize + " but: " + i + " is greater than " + pSize
                        )
                        : DataResult.success(new ChatLog(pSize, (List<LoggedChatEvent>)p_274704_));
                },
    ...
    private ChatLog(int pSize, List<LoggedChatEvent> pEvents) {
        this.buffer = pEvents.toArray(LoggedChatEvent[]::new);
