...
public class LandRandomPos {
        ...
        return RandomPos.generateRandomPos(
            ...
            () -> {
                BlockPos blockpos = RandomPos.generateRandomDirectionWithinRadians(
                    pMob.getRandom(), pRadius, pYRange, 0, pVectorPosition.x, pVectorPosition.z, (float) (Math.PI / 2)
                );
                if (blockpos == null) {
                    return null;
                } else {
    ...
    public static BlockPos movePosUpOutOfSolid(PathfinderMob pMob, BlockPos pPos) {
        pPos = RandomPos.moveUpOutOfSolid(pPos, pMob.level().getMaxY(), p_148534_ -> GoalUtils.isSolid(pMob, p_148534_));
    ...
    public static BlockPos generateRandomPosTowardDirection(PathfinderMob pMob, int pRadius, boolean pShortCircuit, BlockPos pPos) {
        ...
        return !GoalUtils.isOutsideLimits(blockpos, pMob)
                && !GoalUtils.isRestricted(pShortCircuit, pMob, blockpos)
                && !GoalUtils.isNotStable(pMob.getNavigation(), blockpos)
            ? blockpos
            : null;
    }
}