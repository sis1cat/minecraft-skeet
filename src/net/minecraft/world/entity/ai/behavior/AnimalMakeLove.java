...
public class AnimalMakeLove extends Behavior<Animal> {
    ...
    private Optional<? extends Animal> findValidBreedPartner(Animal pAnimal) {
        return pAnimal.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).get().findClosest(p_374978_ -> {
            if (p_374978_.getType() == this.partnerType && p_374978_ instanceof Animal animal && pAnimal.canMate(animal) && !animal.isPanicking()) {
                return true;
            }
