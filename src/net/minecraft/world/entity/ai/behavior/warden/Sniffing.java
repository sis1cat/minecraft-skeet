...
public class Sniffing<E extends Warden> extends Behavior<E> {
    ...
    protected void stop(ServerLevel p_217672_, E p_217673_, long p_217674_) {
        ...
        p_217673_.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE).filter(p_217673_::canTargetEntity).ifPresent(p_375073_ -> {
            if (p_217673_.closerThan(p_375073_, 6.0, 20.0)) {
                p_217673_.increaseAngerAt(p_375073_);
            }

            if (!p_217673_.getBrain().hasMemoryValue(MemoryModuleType.DISTURBANCE_LOCATION)) {
                WardenAi.setDisturbanceLocation(p_217673_, p_375073_.blockPosition());
            }
        });
    }
}