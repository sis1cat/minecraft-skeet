...
public class BlockPattern {
    ...
    protected static BlockPos translateAndRotate(BlockPos pPos, Direction pFinger, Direction pThumb, int pPalmOffset, int pThumbOffset, int pFingerOffset) {
        if (pFinger != pThumb && pFinger != pThumb.getOpposite()) {
            ...
            return pPos.offset(
                vec3i1.getX() * -pThumbOffset + vec3i2.getX() * pPalmOffset + vec3i.getX() * pFingerOffset,
                vec3i1.getY() * -pThumbOffset + vec3i2.getY() * pPalmOffset + vec3i.getY() * pFingerOffset,
                vec3i1.getZ() * -pThumbOffset + vec3i2.getZ() * pPalmOffset + vec3i.getZ() * pFingerOffset
            );
        } else {
