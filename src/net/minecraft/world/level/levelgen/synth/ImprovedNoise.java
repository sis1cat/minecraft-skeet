...
public final class ImprovedNoise {
    ...
    public double noise(double pX, double pY, double pZ) {
        return this.noise(pX, pY, pZ, 0.0, 0.0);
    }
    ...
    public double noise(double pX, double pY, double pZ, double pYScale, double pYMax) {
        ...
        if (pYScale != 0.0) {
            double d7;
            if (pYMax >= 0.0 && pYMax < d4) {
                d7 = pYMax;
            } else {
                d7 = d4;
            }

            d6 = (double)Mth.floor(d7 / pYScale + 1.0E-7F) * pYScale;
        } else {
            d6 = 0.0;
        }
    ...
    private double sampleAndLerp(int pGridX, int pGridY, int pGridZ, double pDeltaX, double pWeirdDeltaY, double pDeltaZ, double pDeltaY) {
        ...
        double d1 = gradDot(this.p(i1 + pGridZ), pDeltaX - 1.0, pWeirdDeltaY, pDeltaZ);
        double d2 = gradDot(this.p(l + pGridZ), pDeltaX, pWeirdDeltaY - 1.0, pDeltaZ);
        double d3 = gradDot(this.p(j1 + pGridZ), pDeltaX - 1.0, pWeirdDeltaY - 1.0, pDeltaZ);
        double d4 = gradDot(this.p(k + pGridZ + 1), pDeltaX, pWeirdDeltaY, pDeltaZ - 1.0);
        double d5 = gradDot(this.p(i1 + pGridZ + 1), pDeltaX - 1.0, pWeirdDeltaY, pDeltaZ - 1.0);
        double d6 = gradDot(this.p(l + pGridZ + 1), pDeltaX, pWeirdDeltaY - 1.0, pDeltaZ - 1.0);
        double d7 = gradDot(this.p(j1 + pGridZ + 1), pDeltaX - 1.0, pWeirdDeltaY - 1.0, pDeltaZ - 1.0);
    ...
    private double sampleWithDerivative(int pGridX, int pGridY, int pGridZ, double pDeltaX, double pDeltaY, double pDeltaZ, double[] pNoiseValues) {
        ...
        double d1 = SimplexNoise.dot(aint1, pDeltaX - 1.0, pDeltaY, pDeltaZ);
        double d2 = SimplexNoise.dot(aint2, pDeltaX, pDeltaY - 1.0, pDeltaZ);
        double d3 = SimplexNoise.dot(aint3, pDeltaX - 1.0, pDeltaY - 1.0, pDeltaZ);
        double d4 = SimplexNoise.dot(aint4, pDeltaX, pDeltaY, pDeltaZ - 1.0);
        double d5 = SimplexNoise.dot(aint5, pDeltaX - 1.0, pDeltaY, pDeltaZ - 1.0);
        double d6 = SimplexNoise.dot(aint6, pDeltaX, pDeltaY - 1.0, pDeltaZ - 1.0);
        double d7 = SimplexNoise.dot(aint7, pDeltaX - 1.0, pDeltaY - 1.0, pDeltaZ - 1.0);
        double d8 = Mth.smoothstep(pDeltaX);
        double d9 = Mth.smoothstep(pDeltaY);
        double d10 = Mth.smoothstep(pDeltaZ);
    ...
    public void parityConfigString(StringBuilder pBuilder) {
        NoiseUtils.parityNoiseOctaveConfigString(pBuilder, this.xo, this.yo, this.zo, this.p);
    }
}