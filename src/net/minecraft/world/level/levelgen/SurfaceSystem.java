...
public class SurfaceSystem {
    ...
    public SurfaceSystem(RandomState pRandomState, BlockState pDefaultBlock, int pSeaLevel, PositionalRandomFactory pNoiseRandom) {
        ...
        this.clayBands = generateBands(pNoiseRandom.fromHashOf(ResourceLocation.withDefaultNamespace("clay_bands")));
    ...
    public void buildSurface(
        ...
        SurfaceRules.RuleSource pRuleSource
    ) {
    ...
    protected int getSurfaceDepth(int pX, int pZ) {
        double d0 = this.surfaceNoise.getValue((double)pX, 0.0, (double)pZ);
        return (int)(d0 * 2.75 + 3.0 + this.noiseRandom.at(pX, 0, pZ).nextDouble() * 0.25);
    }

    protected double getSurfaceSecondary(int pX, int pZ) {
        return this.surfaceSecondaryNoise.getValue((double)pX, 0.0, (double)pZ);
    }
    ...
    public Optional<BlockState> topMaterial(
        ...
        SurfaceRules.Context surfacerules$context = new SurfaceRules.Context(
            this, pContext.randomState(), pChunk, pNoiseChunk, pBiomeGetter, pContext.registryAccess().lookupOrThrow(Registries.BIOME), pContext
        );
    ...
    private void erodedBadlandsExtension(BlockColumn pBlockColumn, int pX, int pZ, int pHeight, LevelHeightAccessor pLevel) {
        double d0 = 0.2;
        double d1 = Math.min(
            Math.abs(this.badlandsSurfaceNoise.getValue((double)pX, 0.0, (double)pZ) * 8.25),
            this.badlandsPillarNoise.getValue((double)pX * 0.2, 0.0, (double)pZ * 0.2) * 15.0
        );
        if (!(d1 <= 0.0)) {
            double d2 = 0.75;
            double d3 = 1.5;
            double d4 = Math.abs(this.badlandsPillarRoofNoise.getValue((double)pX * 0.75, 0.0, (double)pZ * 0.75) * 1.5);
            double d5 = 64.0 + Math.min(d1 * d1 * 2.5, Math.ceil(d4 * 50.0) + 24.0);
            int i = Mth.floor(d5);
            if (pHeight <= i) {
                for (int j = i; j >= pLevel.getMinY(); j--) {
                ...
                for (int k = i; k >= pLevel.getMinY() && pBlockColumn.getBlock(k).isAir(); k--) {
    ...
    private void frozenOceanExtension(
        ...
        double d0 = 1.28;
        double d1 = Math.min(
            Math.abs(this.icebergSurfaceNoise.getValue((double)pX, 0.0, (double)pZ) * 8.25),
            this.icebergPillarNoise.getValue((double)pX * 1.28, 0.0, (double)pZ * 1.28) * 15.0
        );
        if (!(d1 <= 1.8)) {
            double d3 = 1.17;
            double d4 = 1.5;
            double d5 = Math.abs(this.icebergPillarRoofNoise.getValue((double)pX * 1.17, 0.0, (double)pZ * 1.17) * 1.5);
            double d6 = Math.min(d1 * d1 * 1.2, Math.ceil(d5 * 40.0) + 14.0);
            if (pBiome.shouldMeltFrozenOceanIcebergSlightly(pTopWaterPos.set(pX, this.seaLevel, pZ), this.seaLevel)) {
                d6 -= 2.0;
            }
            ...
            for (int l = Math.max(pHeight, (int)d6 + 1); l >= pMinSurfaceLevel; l--) {
                if (pBlockColumn.getBlock(l).isAir() && l < (int)d7 && randomsource.nextDouble() > 0.01
                    || pBlockColumn.getBlock(l).is(Blocks.WATER) && l > (int)d2 && l < this.seaLevel && d2 != 0.0 && randomsource.nextDouble() > 0.15) {
                    if (k <= i && l > j) {
    ...
    protected BlockState getBand(int pX, int pY, int pZ) {
        int i = (int)Math.round(this.clayBandsOffsetNoise.getValue((double)pX, 0.0, (double)pZ) * 4.0);
